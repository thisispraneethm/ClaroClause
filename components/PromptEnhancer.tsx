    ...
    return (
        <div className="w-full max-w-3xl animate-fade-in">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold">Enhance Analysis</h2>
                <p className="text-muted-foreground mt-2">Tailor the AI's focus for more relevant results.</p>
            </div>
            <div className="bg-white/10 border border-white/20 rounded-2xl shadow-xl p-6 md:p-8 backdrop-blur-xl space-y-8">
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {personas.map((p) => (
                            <div key={p.id}>
                                ...
                                <label
                                    htmlFor={p.id}
                                    className={`flex flex-col p-4 border rounded-lg cursor-pointer transition-all duration-200 ${options.persona === p.id ? 'bg-primary/20 border-primary/50 ring-2 ring-primary' : 'bg-white/10 border-white/20 hover:bg-white/20'}`}
                                >
                ...
                <div>
                    ...
                    <textarea
                        ...
                        className="w-full h-24 p-4 bg-white/20 border border-white/30 rounded-lg text-foreground placeholder-muted-foreground focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all duration-300 resize-none shadow-glass"
                    />
                </div>
            </div>
            ...
            <div className="mt-8 flex items-center justify-between">
                <button
                    onClick={onBack}
                    className="inline-flex items-center justify-center gap-2 px-6 py-2 text-sm font-medium rounded-full transition-colors bg-white/20 text-secondary-foreground hover:bg-white/40 border border-white/20"
                >
                ...
                <button
                    ...
                    className={`group relative inline-flex items-center justify-center px-8 py-3 h-12 overflow-hidden rounded-full font-semibold text-white transition-all duration-300 bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:from-violet-600 hover:to-fuchsia-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-background focus:ring-fuchsia-500 shadow-lg shadow-primary/30 animate-pulse-slow`}
                >
